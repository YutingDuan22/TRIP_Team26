...
from ICL.utils import randomly_split
...
def replace_objects_per_example(input_train, replacements):
    """
    Replaces old objects with new ones for specific example IDs.

    Parameters:
        input_train (list): The dataset containing story examples.
        replacements (dict): A dictionary where keys are example IDs and values are
                            tuples of (old_objects, new_objects) for replacement.

    Returns:
        list: The updated input_train with modifications applied.
    """
    for example in input_train:
        example_id = example['example_id']
        if example_id in replacements:
            old_objects, new_objects = replacements[example_id]
            old_objects_lower = [obj.lower() for obj in old_objects]
            new_objects_lower = [obj.lower() for obj in new_objects]
            for story in example['stories']:
                # Replace in "sentences"
                updated_sentences = []
                for sentence in story['sentences']:
                    for old_obj, new_obj in zip(old_objects_lower, new_objects_lower):
                        sentence = sentence.lower().replace(old_obj, new_obj)
                    updated_sentences.append(sentence)
                story['sentences'] = updated_sentences


                # Replace in "states" without duplication
                updated_states = []
                for state in story['states']:
                    updated_state = {}
                    for key, value in state.items():
                        if isinstance(value, list):
                            # Process each item once and replace as needed
                            processed_items = []
                            for item in value:
                                if isinstance(item, list):
                                    for old_obj, new_obj in zip(old_objects_lower, new_objects_lower):
                                        item = [new_obj if obj == old_obj else obj for obj in item]
                                        processed_items.append(item)
                                    
                                else:
                                    processed_items.append(item)
                            # Deduplicate items
                            updated_state[key] = list({tuple(item) for item in processed_items})
                        else:
                            updated_state[key] = value
                    updated_states.append(updated_state)
                story['states'] = updated_states
...
###### for demo_choice == "custom_stories4" in trip_soft_chaining.py: pass replacements dict
def load_trip_dataset_custom_train(replacements, exclude_multi_confl=False, condense_multiple_conflict=False, reduce_options=False):
    raw_all_data = json.load(open("../data/trip.json", "r"))
    raw_train = raw_all_data[0]["train"] # cloze
    
    custom_train = replace_objects_per_example(raw_train,replacements)
    trip_dataset = {}
    print("Loading customed demo train data...")
    trip_dataset["train"] = raw_dict_to_dataset(custom_train, exclude_multi_confl=exclude_multi_confl, condense_multiple_conflict=condense_multiple_conflict, reduce_options=reduce_options)
