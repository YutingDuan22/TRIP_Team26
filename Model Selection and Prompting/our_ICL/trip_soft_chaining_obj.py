...
#from data.trip import load_trip_dataset, load_trip_dataset_custom_train
from data.trip_updated import load_trip_dataset, load_trip_dataset_custom_train
...
if __name__ == '__main__':
    ...
    if args.demo_choice == "stories-4":
        ...
        """
        replacements = {"train_588": (["donut"], ["banana"]),
        "train_107": (['axe', 'scissors', 'rope'],['wrench','flashlight','tape']),
        "train_51":(['hair dryer'],['portable fan']),
        "train_311":(['tv'],['lamp'])}
        """
        replacements = {
            "train_588": (["donut"], ["apple"]),
            "train_107": (["axe", "scissors", "rope"], ["hammer", "pliers", "cord"]),
            "train_51": (["hair dryer"], ["heater"]),
            "train_311": (["tv"], ["projector"])}

        
        train_dataset = load_trip_dataset_custom_train(replacements, exclude_multi_confl=False, condense_multiple_conflict=False, reduce_options=False)["train"]
    ...
    elif args.demo_choice =="custom_opt_rep":
        selected_story_example_ids = ['train_1', 'train_8', 'train_691', 'train_693']
        replacements = {
            "train_1": (
                [ "microwave",  "plate", "soup"],
                [ "oven",  "bowl",  "stew"]),
                
            "train_8": (["microwave",  "banana"],["toaster", "apple"]),
            
            "train_691": (["bin"], ["basket"]),
            
            "train_693": (["bread", "sandwich"],["pita", "wrap"])
            }
        train_dataset = load_trip_dataset_custom_train(replacements, exclude_multi_confl=False, condense_multiple_conflict=False, reduce_options=False)["train"]
