...
if __name__ == '__main__':
    ...
    ### add role 
    parser.add_argument("--role_choice", type=str,default="chatbot", choices=["Careful_Story_Editor", "Interior_Decorator"])#*
    ...
    cache_dir = os.path.join(current_dir, "cache_files/soft_chain")
    ...
    CACHE_FNAME = os.path.join(cache_dir, f'trip_lm_cache_{args.lm_backbone}_{args.demo_choice}_{args.reasoning_depth}_{args.reasoning_direction}.pkl')
    ...
    if args.lm_backbone in ['llama7B', 'llama13B', 'llama30B', 'llama65B',"Llama-3.1-8B-Instruct"]:
        ...
        if not args.skip_prompting and not args.cache_only:
            ...
            model_path = args.model_path if args.model_path is not None else "meta-llama/Llama-2-7b-hf"
    ...
    if args.lm_backbone == "Llama-3.1-8B-Instruct":
        ### add 3 roles
        if args.role_choice == "Careful_Story_Editor":
            role_prompt = "You are a careful story editor, and you excel at meticulously analyzing narratives for logical consistency and causal relationships."
        elif args.role_choice == "Interior_Decorator":
            role_prompt = "You are an interior decorator, and you excel at analyzing room layouts and ensuring objects are arranged logically and used appropriately."
        else:
            role_prompt = "You are a smart chatbot capable of physical commonsense reasoning, and understanding the physical state of the world implied in written texts. "
                ...
                if args.lm_backbone in ['gpt3', "llama7B", "llama13B", "llama30B", "llama65B","Llama-3.1-8B-Instruct"]:
                    ...
                    if args.lm_backbone in ["llama7B", "llama13B", "llama30B", "llama65B","Llama-3.1-8B-Instruct"]:
                        ...
                        if args.lm_backbone =="Llama-3.1-8B-Instruct":
                            ...
                            llama3_prompt = [{'role': 'system', 'content': role_prompt},
                            {'role': 'user', 'content': prompt}
                            ]
                            story_generated_text = prompt_llama_with_caching_llama3(generator, llama3_prompt, args, lm_cache, example_id, max_tokens=128)
