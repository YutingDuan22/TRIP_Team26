...
import sys
from ICL.visualization import get_output_line_sep, calc_attn

from ICL.utils import imload
...
############## for Llama-3.1-8B-Instruct: original: temperature=0.0 too strict####################
"""
def prompt_hf_llama3(generator, prompt, max_tokens=128, temperature=0.0):
    '''
    ...
    try:
        ...
        if not response or "generated_text" not in response[0]:
            print(f"Error: Invalid response from generator: {response}")
            return ""
        
        return response[0]["generated_text"]
    except Exception as e:
        print(f"Error during text generation: {e}")
        print(f"Prompt causing error: {prompt}")
        return ""
"""

############## for Llama-3.1-8B-Instruct: remove: temperature=0.0####################

def prompt_hf_llama3(generator, prompt, max_tokens=128):
    '''
    Use the Hugging Face pipeline for text generation.
    Args:
        generator: The text generation pipeline.
...
def prompt_llama_with_caching_llama3(generator, prompt, args, lm_cache, cache_key, max_tokens=128):
    """
    Prompt using Llama with caching and the generator pipeline.
    Args:
        generator: The text generation pipeline.
        prompt (str): Input prompt.
        args: Command-line arguments.
        lm_cache (dict): Cache dictionary.
        cache_key (str): Key for caching.
        max_tokens (int): Maximum number of tokens to generate.

    Returns:
        str: Generated or cached text.
    """
    if not args.skip_prompting:
        if cache_key not in lm_cache:
            # Generate text if not cached
            generated_text = prompt_hf_llama3(generator, prompt, max_tokens=max_tokens)
            if generated_text:
                lm_cache[cache_key] = generated_text
                pickle.dump(lm_cache, open(args.cache_fname, 'wb'))
        ...
        print("Prompting skipped. Returning default response.")
        pprint(prompt)
        return "Story A"
